...
while (1) {
    int16_t current_temp = therm_read();
    int16_t setpoint = pot_read();
    int value = flame_detect();
    ...
    switch (currentState) {
        ...
        case HEATING:
            if (!flame_detect()) {
                pilotvalve_off();
                MainValve_set(500);
                rgb_setColor(0, 0, 255); // Blue for Failure (if flame is lost)
                currentState = IDLE;
                break;
            }
            ...
            if (current_temp >= setpoint) {
                pilotvalve_off();
                MainValve_set(500);
                rgb_setColor(0, 255, 0); // Green for IDLE when temp is reached
                currentState = IDLE;
            }
            break;

        default:
            rgb_setColor(0, 0, 255); // Blue for any other states or failure
            break;
    }
